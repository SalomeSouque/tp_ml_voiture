...
def save_prediction (accident, result, metadonnee) :
    db = next(get_db())
    try :
        data_dict = {**accident.model_dump(), **result, **metadonnee}
        ##** = méthode python qui permet directement d'assigner les valeurs
        # qd c'est un dict le contenu de la varibale

        # Convertir booléens correctement parceque j'ai plein de problème
        bool_cols = ['presence_enfant',
                     'presence_senior',
                     'presence_pieton',
                     'presence_passager',
                     'nuit',
        ...
        new_pred = AccidentPrediction(**data_dict)
        #Appel de ma class sqlachemy
        db.add(new_pred)
        db.commit()
    finally :
        db.close()
